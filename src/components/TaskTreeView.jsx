  ...
  const renderNode = (task) => {
    ...
    return (
      <div key={task.id} className="tree-node-wrapper">
        <div 
          className={`list-task-row tree-row ${task.isContextOnly ? 'context-only' : ''}`}
          style={{ 
        ...
        {/* Children rendering would go here if we were doing a recursive DOM structure, 
            but sortByHierarchy already flattens it for us. 
            We just need to handle the collapse logic in the flattened array.
        */}
      </div>
    );
  };
