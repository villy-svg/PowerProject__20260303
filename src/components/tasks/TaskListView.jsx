...
const TaskListView = ({
  ...
  setExpandedTaskId,
  groupByHubs = false,
  allTasks,          // Full unfiltered task list for parent lookups in hub resolution
}) => {
  ...
  /**
   * Renders the stage sections for a given subset of tasks.
   * Extracted so it can be called per-hub when groupByHubs is ON,
   * or directly on the full task list when it's OFF.
   */
  const renderStages = (tasksToRender) => stageList.map((stage) => {
    // Find tasks that belong to this stage section based on their top-level ancestor
    const rawStageRootTasks = tasksToRender.filter(t => !t.parentTask && t.stageId === stage.id);

    // Find all descendants of these roots
    const allMemberIds = new Set();
    rawStageRootTasks.forEach(root => {
      allMemberIds.add(root.id);
      const descendants = hierarchyUtils.getDescendants(tasksToRender, root.id, 'id', 'parentTask');
      descendants.forEach(d => allMemberIds.add(d.id));
    });

    const rawStageTasks = tasksToRender
  ...
  // Pre-compute hub map once when groupByHubs is ON.
  // Uses allTasks (full unfiltered list) for parent lookups so subtasks
  // inherit their parent's hub even when the parent is filtered out.
  const taskHubMap = groupByHubs
    ? new Map(tasks.map(t => [t.id, taskUtils.getTaskHub(t, allTasks || tasks)]))
    : null;
  ...
  return (
    <div className="task-list-view">
      {groupByHubs ? (
        /* ── Hub-Grouped Mode ────────────────────────────────────────────
           Hub Header → Stage Header → Tasks (3-level hierarchy) */
        uniqueHubs.map(hub => {
          const hubTasks = tasks.filter(t => taskHubMap.get(t.id) === hub);
