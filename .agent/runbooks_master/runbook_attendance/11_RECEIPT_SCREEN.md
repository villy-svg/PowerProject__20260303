# Phase 4.2 — Receipt Screen & WhatsApp Share

## Skills Required (Read Before Starting)
- `hybrid-mobile-deployment` §4 (Platform guards — Web Share API only on supported browsers)
- `ui-design-system` §12 (Premium design — glassmorphism, squircle cards)
- `ui-design-system` §14B (Touch targets ≥ 44px)
- `safe-code-modification` §2 (No inline styles, CSS classes only)

---

## Objective

Create `AttendanceReceiptScreen.jsx` — the full-screen digital receipt shown immediately after a successful check-in or check-out. It displays:
- Action confirmation (✅ Shift Started / 👋 Shift Ended)
- Employee Name, Shift Type, Hub Name
- Timestamp, Geolocation tag (if available), Device ID
- A "Share to WhatsApp" button

### WhatsApp Share Strategy

```
Primary:   navigator.share() — Web Share API (Chrome Android, Safari iOS)
Fallback:  window.open('https://wa.me/?text=...') — deep link
```

---

## Step 1: Create `AttendanceReceiptScreen.jsx`

**File to create:**
```
src/verticals/Employees/attendance/AttendanceReceiptScreen.jsx
```
...
- [ ] `navigator.share` is wrapped in an `if (navigator.share)` guard (platform check)
- [ ] `AbortError` is caught and silently ignored (user cancelled share)
- [ ] WhatsApp fallback uses `https://wa.me/?text=` URL (not `whatsapp://` which blocks on web)
- [ ] `buildShareText` is a pure utility function (no side effects, easy to test)
- [ ] `employeeName` and `hubName` are derived safely with fallbacks
- [ ] `deviceId` is truncated for display (`slice(-12)`) to avoid exposing full IDs
- [ ] `onDone` button returns to the self-service check-in form state
- [ ] `noopener,noreferrer` included in `window.open()` for security
- [ ] `useMemo` used for `shareText` to prevent rebuilding on every render
- [ ] NO inline styles

---

## DO NOT Proceed to Phase 5 Until All Items Above Are Checked.
