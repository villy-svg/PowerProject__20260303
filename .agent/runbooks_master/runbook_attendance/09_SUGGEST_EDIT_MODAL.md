# Phase 3.4 — Suggest Edit Modal (Contributor/Maker Flow)

## Skills Required (Read Before Starting)
- `rbac-security-system` §2 (Contributor = Maker — submit only, no direct edit)
- `ui-design-system` §5 (Modal overlay pattern)
- `ui-design-system` §9 (Block-in-a-Box form styling)
- `ui-design-system` §10 (Master Dropdown System for status selector)
...
Create `AttendanceSuggestEditModal.jsx` — the modal a **Contributor** sees when they click any grid cell. They can suggest a status change (e.g., mark a future date as week-off, or flag a missed logout). The suggestion goes into `attendance_edit_requests` with `status = 'pending'` and does NOT touch the live board until an Editor approves it.

---

## Step 1: Create `AttendanceSuggestEditModal.jsx`

**File to create:**
```
src/verticals/Employees/attendance/AttendanceSuggestEditModal.jsx
```
...
- [ ] Form uses `form-group` + `form-input-container` classes (ui-design-system §9)
- [ ] Status select and Shift Type select use `master-dropdown` class (ui-design-system §10)
- [ ] `submitEditRequest` called from `editRequestService.js` (not direct Supabase)
- [ ] `dailyAttendanceId` is `null` if `record?.id` doesn't exist (future date support)
- [ ] `requestedBy` uses `currentUser?.id` (safe optional chaining)
- [ ] Click on overlay closes modal; click inside modal stops propagation
- [ ] Shift type field is hidden when suggested status is not 'present'
- [ ] `isSubmitting` disables both submit and cancel buttons during API call
- [ ] NO inline styles anywhere

---

## DO NOT Proceed to Phase 4 Until All Items Above Are Checked.
